...
def get_interf_res(atomlist, reftree, model):
    ...
    rnainterf = rnatree.sparse_distance_matrix(interftree, args.cutoff)
    A = rnainterf.toarray()
    A[A==0] = 99
...
def process_nachain(chain, struc, rnatype, rnaatoms):
    ...
    for res in chain:
        ...
        resname = rr.strip()
        if resname not in included:
            print("exclude: '%s in %s'"%(resname, struc), file=sys.stderr)
            continue
        NA_c.append(res)
        for a in res:
            nat = a.get_id()
            ...
            rnaatoms.append(a)
            rnatype.append(missingcode[a.get_name()])
    return rnatype, rnaatoms, NA_c
...
def process_prot_interf(rnatype, protmin, resid_c, indexres_c):
    assert 2 * len(resid_c) == len(indexres_c), (len(resid_c), len(indexres_c))
    d_interf = {}
    for k in ['ph', 'sug', 'base']:
        mask = [ j == k for j in rnatype]
...
def process_het_interf(rnaatoms, hetmin, resid_c, indexres_c):
    assert 2 * len(resid_c) == len(indexres_c), (len(resid_c), len(indexres_c))
    d_hetatm = {}
    maskrna = [ True for j in rnaatoms ]
...
for struc in inlist:
    ...
    if not os.path.exists(args.inpdir+"/"+struc+'.pdb'):
        print("ERROR: "+args.inpdir+"/"+struc+".pdb does not exist", file=sys.stderr)
        continue
    try:
        ...
        for model in structure:
            ...
            mm = "model_%i"%m
            NA = {}
            rnaatoms, protatoms, hetatoms, rnatype = [], [], [], []
            for chain in model:
                c = chain.get_id()[0]
                if c in nachains:
                    rnatype, rnaatoms, NA_c = process_nachain(chain, struc, rnatype, rnaatoms)
                    NA[c] = NA_c
                else:
                    hetatoms, protatoms = process_protchain(chain, hetatoms, protatoms)
            rnaarray = np.array([a.get_coord() for a in rnaatoms])
            rnatree = KDTree(rnaarray)
            protmin, _ , prot_interf = get_interf_res(protatoms, rnatree, model)
            hetmin, _ , het_interf = get_interf_res(hetatoms, rnatree, model)
            ...
            for c in nachains:
                ...
                d_prot_interf = process_prot_interf(rnatype, protmin, resid[c], indexres[c])
                ...
                if len(het_interf) > 0:
                    d_het_interf = process_het_interf(rnaatoms, hetmin, resid[c], indexres[c])
