...
ctx.data_parser = transformer(
{
"tailstr": {"pin": "input", "dtype":"str"},
"assembly_scheme": {"pin": "input", "dtype":"int"},
"cutoff": {"pin": "input", "dtype":"float"},
"result": {"pin": "output", "dtype":"text"},
})
ctx.data_parser.result.connect(ctx.csv)
ctx.data_parser_code = ctx.data_parser.code.cell()
ctx.links.data_parser_code = link(ctx.data_parser_code, ".", "cell-data-parser.py")
ctx.generate_attrib = transformer({"csv": {"pin":"input", "dtype": "text"}, "attrib": {"pin":"output", "dtype": "json"}})
ctx.generate_attrib.attrib.connect(ctx.attrib)
ctx.csv.connect(ctx.generate_attrib.csv)
ctx.code_generate_attrib = ctx.generate_attrib.code.cell()
ctx.links.code_generate_attrib = link(ctx.code_generate_attrib, ".", "cell-generate-attrib.py")

ctx.axmax = cell("float").set(60)
ctx.generate_layout = transformer({"axmax": {"pin":"input", "dtype": "float"}, "layout": {"pin":"output", "dtype": "json"}})
ctx.generate_layout.layout.connect(ctx.plotly_layout)
ctx.axmax.connect(ctx.generate_layout.axmax)
ctx.code_generate_layout = ctx.generate_layout.code.cell()
ctx.links.code_generate_layout = link(ctx.code_generate_layout, ".", "cell-generate-layout.py")

ctx.tailstr = cell("str")
ctx.tailstr.connect(ctx.data_parser.tailstr)
ctx.edit.tailstr=combobox("str", ["", "results-inc-ov-aa-sum", "results-inc-ov-aa-sumsum"], "tailstr")
ctx.edit.tailstr.value.connect(ctx.tailstr)

ctx.assembly_scheme = cell("str")

ctx.assembly_scheme.connect(ctx.data_parser.assembly_scheme)
ctx.edit.assembly_scheme=combobox("int", [-1, -2], "assembly scheme")
ctx.edit.assembly_scheme.value.connect(ctx.assembly_scheme)


ctx.cutoff = cell("float")
ctx.cutoff.connect(ctx.data_parser.cutoff)

ctx.edit.cutoff = combobox("float", ["", 2.0, 3.0, 5.0], title="Good structure cutoff")
ctx.edit.cutoff.value.connect(ctx.cutoff)

ctx.axmax = cell("float")
ctx.axmax.connect(ctx.generate_layout.axmax)

ctx.edit.axmax = combobox("float", ["", 5.0, 60.0, 100.0], title="Axmax")
ctx.edit.axmax.value.connect(ctx.axmax)
