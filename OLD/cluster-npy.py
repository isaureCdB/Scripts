...
'''
Slow optimised clustering:
Starts clustering around the most connected structure
'''
filename = sys.argv[1]       # chains[.npy]
cutoff = float(sys.argv[2])     # 2 A

outp1 = filename + '-clust' + str(cutoff) + '.npy'         # chains-clust2
outp2 = filename + '-clust' + str(cutoff)

pdb = np.load(filename+'.npy')
if len(pdb.shape) == 3:
...
if False:
    rmsdfile = filename+'.lrmsd'
    rmsdlist = [ float(l.split()[1]) for l in open(rmsdfile).readlines()  ]
    outp3 = filename + '-clust' + str(cutoff) + '.rmsd-per-clustsize'
    outp4 = filename + '-clust' + str(cutoff) + '.rmsd-per-rank'
    out3 = open(outp3,"w")
    out4 = open(outp4,"w")
