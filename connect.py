...
Argument 5 to (4 + nfrags - 1): the "preatoms" portion of atom coordinates of the poses (the atoms that overlap with the next fragment)
Argument (5 + nfrags) to (5 + 2 * nfrags - 1): the "postatoms" portion of atom coordinates of the poses (the atoms that overlap with the previous fragment)
...
Argument (4 + 2 * nfrags) - (4 + 3 * nfrags): optional: lists of pose indices to select for each fragment

Copyright 2015-2017 Sjoerd de Vries, Isaure Chauvot de Beauchene, TUM
"""
import sys, json, numpy as np, bisect
import connectlib
...
if __name__ == "__main__":
    if len(sys.argv) < 8:
        raise Exception("usage: connect.py nfrags rmsd maxstruc minchild [preatoms.npy] [postatoms.npy] [sel]")
    ...
    for a in postatoms: assert len(a.shape) == 2 and a.shape[1] % 3 == 0, a.shape
    for a in preatoms: assert len(a.shape) == 2 and a.shape[1] % 3 == 0, a.shape
    ...
    json.dump(tree, sys.stdout, sort_keys=True)
