...
parser.add_argument('--motifs', help="index of the motif in motifs list: \
                    ex: 011 for AUU + UUU + UUU", nargs='+', type=int)
...
nat = args.nat # 7666 for AUUU
m = args.motifs # motifs: 011 for AUU+UUU+UUU
assert nfrag == len(nat) - 2 and len(m) == nfrag, (m, nfrag, nat)
