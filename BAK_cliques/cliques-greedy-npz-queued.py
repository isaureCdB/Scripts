...
def subdivide(count, superclust, subcenters, threshold, step, tree):
    clique = tree[step][count]
    sub_indices = []
    ...
    tree[step][count] = map_cliques

def create_cliques(step, interactions, pool, tree):
    superclust, subcenters, radius = superclusts[step], subcenterss[step], radii[step]
    threshold = cutoff**2 + 2*radius**2
    to_subdivide = [ tree[i] for i in tree.keys() if len(i) < len(radii) ]
    count = 0
    while len(to_subdivide) > 0:
        ...
        while len(jobs) < np:
            p = multiprocessing.Process(target=subdivide,/
                args=(count, superclust, subcenters, threshold, step, tree))
            jobs.append(p)
            p.start
            count += 1
        for j in jobs:
    ...
    for cl in tree[step+1][:-1]:
        print >> sys.stderr, len(cl),
    print >> sys.stderr, len(tree[step+1][-1])
    return tree
...
if __name__ == '__main__':
    ...
    tree = {}
    tree{0} = range(len(superclusts[0]))
    for i in range(len(radii)):
        tree = create_cliques(i, interactions, pool, tree)
