...
def get_connected(poses, sel, sel1_fwd, sel1_bwd):
    # poses : list of poses in the pre-clique, indexed in the
    # "sel" selection of poses
    abs_poses = [ sel[i] for i in poses ] # in absolute indexing
    nfrags = npz["nfrags"]
    #
    sel_fwd = [ set() for p in poses]
    for frag in range(nfrags-1):
        sel1 = [ set([p]) for p in poses]
...
def process_clique(clique, superclust, supercenters, subcenters, npy, threshold):
    ...
    if len(subcenters) == nsel: # we are at the singleton level
        connected = get_connected(poses, sel, sel1_fwd, sel1_bwd)
    ### TODO ####
    ...
    return map_cliques, supercenters
...
def create_cliques(step, cliques, npy):
    ...
    if 0:
        ...
        for clique in cliques:
            all_map_cliques.append(process_clique(clique, superclust, supercenters, subcenters, npy, threshold))
    elif 0:
        ###   Sequential (functional programming):
        def process_clique_wrapper(clique):
            return process_clique(clique, superclust, supercenters, subcenters,  npy, threshold)
