...
##########################################################################
  ...
  #   - structures.json
  #!!! It deletes pdb if already present in structures.json !!
python3 $NAFRAG/interface_pdb_contacts.py 5 brutPDBs chainsmodels checked.list \
  $ATTRACTTOOLS/../allatom/$na-mutate.list structures.json $na \
...
$NAFRAG/clean_rna.py 'chainsmodels/' 'cleanPDB/' cleanPDB.list \
  $ATTRACTDIR/../allatom/$na-mutate.list structures.json \
...
##########################################################################
  ...
  ### Adds entries in structures.json:
