...
set -u -e
# create PDB templates for numpy to PDB conversions
if [ ! -s templates/$m\r.pdb ];then
    echo "#create templates $m"
    $ATTRACTDIR/center templates/$m.pdb > /tmp/$m-c.pdb
    mv /tmp/$m-c.pdb templates/$m.pdb
    python2 $ATTRACTTOOLS/reduce.py templates/$m.pdb > /dev/null
fi
...
rm $m-aa-fit-clust${dr}.npy $m-aa-fit-clust${dr}
