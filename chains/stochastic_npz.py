...
import math
import random
from npy import map_npz

def map_json(jsonfile):
    j = np.load(jsonfile)
    nfrags = j['nfrags']
    interactions = [j['interactions-%i'%i] for i in range(nfrags-1)]
    clusters = [j['clusters-%i'%i] for i in range(nfrags)]
...
if __name__ == "__main__":
	...
	interactions, poses = map_json(json_file)
