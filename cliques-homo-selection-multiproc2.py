...
if __name__ == '__main__':
    ...
    sel = [int(i)-1 for i in open(sys.argv[2]).readlines()] # poses.list
    cutoff = float(sys.argv[3]) #cutoff (Angstrom) to consider atoms as clashing
    radii = [float(i) for i in open(sys.argv[4]).readlines()[0].split()] #list of the clustering radius (Angstrom)
